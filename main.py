...
class card_prices_thread(threading.Thread):
    def __init__(self,card):
        threading.Thread.__init__(self)
        self.card = card

    def run(self):
        self.get_card_price(self.card)
...
def get_card_prices(deck):
    ...
    threads = []
    for c in deck['cards']:
        print(f"({deck['cards'].index(c) + 1}/{len(deck['cards'])}) - {c['name']}")
        t = card_prices_thread(c)
        t.start()
        threads.append(t)
...
#    output_list(card_list_prices,sys.argv[2])
    ...
    output_store_list(STORES,sys.argv[2])
